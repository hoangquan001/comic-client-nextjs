...
import { useState, useEffect, useRef, useCallback, ReactNode, useImperativeHandle } from 'react';
    ...
    if (containerRef.current) {
      if (selectedID != null) {
        const idx = allItems.findIndex((item) =>
          trackById ? trackById(item) === selectedID : (item as any).id === selectedID,
        );
      ...
      <div style={{ height: totalHeight, position: 'relative' }}>
        ...
        >
          {visibleItems.map((item, i) => (
            <div key={trackById ? trackById(item) : i} style={{ height: itemHeight }}>
              {renderItem(item, i)}
            </div>
          ))}
        </div>
