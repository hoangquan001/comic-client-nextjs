...
const ITEMS: InventoryItem[] = [
  { id: 1, name: 'Huy hiệu Đọc giả', description: 'Đọc 100 chương truyện', image: '/emoji/1.gif', rarity: 'common', category: 'badge', quantity: 1 },
  { id: 2, name: 'Huy hiệu Bình luận', description: 'Bình luận 50 lần', image: '/emoji/2.gif', rarity: 'rare', category: 'badge', quantity: 1 },
  { id: 3, name: 'Huy hiệu Theo dõi', description: 'Theo dõi 20 truyện', image: '/emoji/3.gif', rarity: 'epic', category: 'badge', quantity: 1 },
];
...
export default function TuiDoContent() {
  ...
  const filteredItems = useMemo(() => {
    const baseItems = activeTab === 'equipped' ? ITEMS.filter((item) => item.isEquipped) : ITEMS;
  ...
  }, [activeTab, category, rarity, searchTerm]);
  ...
  return (
    <div className="mx-auto max-w-7xl space-y-6">
      ...
      <div className="flex flex-wrap gap-2 rounded-lg bg-white p-2 dark:bg-neutral-800">
        <TabButton active={activeTab === 'owned'} icon="🎒" label="Đang sở hữu" count={ITEMS.length} onClick={() => setActiveTab('owned')} />
        <TabButton active={activeTab === 'equipped'} icon="⚔️" label="Đang trang bị" count={ITEMS.filter((item) => item.isEquipped).length} onClick={() => setActiveTab('equipped')} />
        <TabButton active={activeTab === 'all'} icon="📋" label="Toàn bộ items" count={ITEMS.length} onClick={() => setActiveTab('all')} />
      </div>
      ...
      <div className="grid grid-cols-1 gap-4 rounded-lg bg-white p-6 dark:bg-neutral-800 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6">
        ...
        <FilterGroup label="Loại vật phẩm">
          <select value={category} onChange={(event) => setCategory(event.target.value)} className="w-full rounded-md border border-neutral-300 bg-white px-3 py-2 text-sm text-neutral-900 focus:border-sky-500 focus:outline-none focus:ring-2 focus:ring-sky-500 dark:border-neutral-600 dark:bg-neutral-700 dark:text-neutral-100">
  ...
  return (
    <div className="relative min-h-[120px] cursor-pointer rounded-lg border border-neutral-200 bg-white p-3 transition-all duration-200 ease-in-out hover:scale-105 hover:border-neutral-300 hover:shadow-md dark:border-neutral-700 dark:bg-neutral-800 dark:hover:border-neutral-600">
      ...
      <div className="min-w-0">
        ...
        <div className="mb-1">
          <span className={`text-xs font-medium ${rarity.text}`}>{rarity.displayName}</span>
        </div>
